...
| --- | --- | --- |
| Test ``PostService``: a) with mocked auth service; b) with mocked data source (with field-by-field input/output data comparison) | ``src/services/post-service.unit.test.ts`` | ``MockPostService`` (mock data source) >> ``MockAuthenticationService`` (mock token validation). |
| Test ``GraphQLServer``: with mocked GraphQL client and mocked data source (with field-by-field input/output data comparison) | ``test/integration/mock/graphql-server.int.test.ts`` | ``MockGraphQLClient`` >> ``GraphQLServer`` >> [``MockAuthenticationService`` + ``MockPostService``].
